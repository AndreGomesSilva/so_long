...
/*   Updated: 2023/09/14 13:04:34 by angomes-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!create_new_player_image(window, path))
	{
		free_game(window, game);
		exit(EXIT_FAIL);
	}
	mlx_image_to_window(window->mlx, window->player_img, \
	game->player_x * IMAGE_WIDTH, game->player_y * IMAGE_HEIGHT);
...
void	player_move_down(t_window *window, t_game *game)
{
	game->player_y += 1;
	update_player_movement(window, game, \
	"./textures/player_idle/down/tile000.png");
	if (game->map[game->player_y][game->player_x] == 'C')
		collect(window, game);
	else if (game->collectable_number <= 0 && \
	game->map[game->player_y][game->player_x] == 'E')
